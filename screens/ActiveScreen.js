...
export default function ActiveScreen(props) {
  ...
  const onSubmitTodo = () => {
    ...
    const newTodoList = [...completeTodoList, newTodo];
  ...
  const onDeleteTodo = id => {
    const newTodoList = completeTodoList.filter(todo => todo.id !== id);
  ...
  const onToggleTodo = (id) => {
    ...
    const todo = completeTodoList.find(todo => todo.id === id);
    todo.status = todo.status === 'Done' ? 'Active' : 'Done';
    const foundIndex = completeTodoList.findIndex(todo => todo.id === id);
    completeTodoList[foundIndex] = todo;
    const newTodoList = [...completeTodoList];
